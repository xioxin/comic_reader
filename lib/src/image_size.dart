...
class ImageSize {
  ...
  static String? getType(Uint8List buffer) {
    ...
    final types = Set.from(headers.keys)
      ..removeAll(not);
    return types.first;
  }
  ...
  static ImageSize? getSize(Uint8List buffer) {
    ...
    switch(type) {
      case 'png':
        return ImageSize(blob.getUint32(0x10, Endian.big), blob.getUint32(0x14, Endian.big));
      case 'gif':
        return ImageSize(blob.getUint16(6, Endian.little), blob.getUint16(8, Endian.little));
      case 'webp':
        return ImageSize(blob.getUint16(0x1a, Endian.little), blob.getUint16(0x1c, Endian.little));
      case 'jpg':
        var offset = 2;
        while (true) {
          ...
          if(marker == 0xFFC0 || marker == 0xFFC2) {
            return ImageSize(blob.getInt16(offset + 7, Endian.big), blob.getInt16(offset + 5, Endian.big));
          }
          offset++;
        }
    }
